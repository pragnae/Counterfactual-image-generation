...
if __name__ == "__main__":
    ...
    if config.load_pretrained:
        ...
        if config.shift_predictor_model == 1:
            shift_model = MLP(config.latent_size, len(config.subset_labels), 1024, config.latent_size)
        elif config.shift_predictor_model == 2:
            shift_model = MLPDropout(config.latent_size, len(config.subset_labels), 1024, config.latent_size)
        elif config.shift_predictor_model == 3:
            shift_model = MLPThreeLayer(config.latent_size, len(config.subset_labels), 1024, config.latent_size)
        elif config.shift_predictor_model == 4:
            shift_model = MLPThreeLayerDropout(config.latent_size, len(config.subset_labels), 1024, config.latent_size)
        else:
            shift_model = MLPDropout(config.latent_size, len(config.subset_labels), 1024, config.latent_size)
